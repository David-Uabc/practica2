package pract2;
//import java.util.ArrayList; tenia much en mente este paquete para capturar pero no supe como utilizarlo ya que creo que es mucho mas facil para
...
public class Main {
  ...
  opc = CapturaEntrada.capturarEntero("------>Opcion a consultar");
  switch(opc)
 {
  case 1:                
  ...
  for(int i = 5; i<10;i++)
 {
  ...
  CapturaEntrada.capturarFlotante("Calificacion"));
 }
  break;
 ...
 case 3: //Cambiar calificacion //solo cambiaremos las calificaciones de los capturados y si quiere cambiar la de los- 
	 // capturados cambie dentro del for donde dice int i=5 a int i=0 para que los tome encuenta

  for(int i = 5;i<10;i++){
	  
  Alumnos[i].setCalificacion(CapturaEntrada.capturarFlotante("Introduce calficacion nueva para " + Alumnos[i].getNombre()));
}
 ...
 break;
  ...
  System.out.println("*************Gracias por visitar el programa************* ");
    ...
    //metodo para asignar el estado del estudiante mediante el ingreso de su calificacion 
    public String asignarEstado() 
    {
        if(calificacion >= 60){estado = "Aprobado";
        }else{estado = "Reprobado";
        }
        return estado;
    }
    
    public void cambiarCalificacion(float CaliNueva) //Metodo que cambia la calificacion del alumno
    {
